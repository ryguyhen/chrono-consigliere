...
export default async function RollPage({ searchParams }: PageProps) {
  ...
  const activeTab = searchParams.tab === 'friends' ? 'friends' : 'mine';
  ...
  if (activeTab === 'mine') {
    [collections, totalSaves, saves] = await Promise.all([
  ...
  if (activeTab === 'friends') {
    [feedEvents, following, suggestedUsers] = await Promise.all([
    ...
    trendingBrands = await prisma.watchListing.groupBy({
      by: ['brand'],
      where: {
        isAvailable: true,
            ...
            <div>
              {feedEvents.length === 0 ? (
                <div className="text-center py-24">
                  ...
                  <Link href="/people" className="font-mono text-[10px] tracking-[0.1em] uppercase text-gold hover:text-gold-dark transition-colors">
                    Find people →
                  </Link>
                </div>
              ) : (
                  ...
                  <div className="space-y-4">
                    {suggestedUsers.map((user: any) => (
                      <div key={user.id} className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="text-[13px] text-ink truncate">{user.profile?.displayName ?? user.name}</div>
                          {user.profile?.tasteTags?.slice(0, 2).length ? (
                            <div className="text-[10px] text-muted truncate">{user.profile.tasteTags.slice(0, 2).join(', ')}</div>
                          ) : null}
                        </div>
                        <form action={`/api/follow/${user.id}`} method="POST">
                          <button type="submit" className="font-mono text-[9px] tracking-[0.1em] uppercase px-2.5 py-1 border border-[var(--border)] rounded hover:border-gold/60 hover:text-gold text-muted/70 transition-colors">
                            Follow
                          </button>
                        </form>
                      </div>
